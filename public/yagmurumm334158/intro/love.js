...
const DEFAULT_HINT = 'Yayı germek için bas, fırlatmak için bırak ♡';
const MISS_HINTS = ['Az kaldı — tekrar dene ♡', 'Kalbi hedefle ♡', 'Aşk Tanrısı iki kez ıskalamaz ♡'];
const ARROW_LEN  = 56;
...
function sayYes(){
  catState = 'happy';
  title.textContent = 'Yaşasın! Biliyordum ♡';
  drawSprite(catCanvas, CAT_HAPPY);
  catCanvas.setAttribute('aria-label', 'Fiyonklu mutlu bir kedi');
  ...
  setTimeout(() => continueBtn.classList.add('is-show'), reduceMotion ? 0 : 900);
}

yesBtn.addEventListener('click', sayYes);

continueBtn.addEventListener('click', () => {
  window.parent.postMessage('yagmurumm-proposal-intro:done', window.location.origin);
});
...
function reset(){
  catState = 'idle';
  title.textContent = 'Yağmurum,\nhâlâ seviyor musun beni? ♡';
  drawSprite(catCanvas, CAT_IDLE);
  catCanvas.setAttribute('aria-label', 'Kalp tutan küçük bir kedi');
  buttons.hidden = false;
  finalText.hidden = true;
  finalText.classList.remove('is-show');
  continueBtn.hidden = true;
  continueBtn.classList.remove('is-show');
